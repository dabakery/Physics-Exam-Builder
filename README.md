...
The published page is generated by [`scripts/build_standalone_html.py`](./scripts/build_standalone_html.py) from the enhanced template: `docs/standalone-enhanced.html` (Markdown/LaTeX/`.zip`/`.doc` export and live GitHub bank fetch). Cloudflare Pages runs the build itself on every push to `main`, so the generated page is **not committed** — it is in `.gitignore`. The script can still produce the base `docs/standalone.html` from `frontend/index.html`, but that page is linked from nowhere and is no longer built on deploy.
